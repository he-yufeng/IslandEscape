...
During the final development stage, we extended the project with a boss dungeon
challenge, improved the negotiation interface, added clearer phase and resource
feedback, and tightened validation across linting, type checking, tests, and
builds. The resulting prototype demonstrates an integrated game loop: movement,
resource labor, LLM-based negotiation, persistent game state, real-time AI turn
events, and an optional action-combat challenge that creates a stronger final
demo arc.
\end{abstract}
...
\begin{enumerate}
  \item the day starts and merchant prices refresh;
...
actions, game state, SSE events, and dungeon results are all described in shared
schemas. The server validates incoming actions against those schemas, while the
frontend receives inferred TypeScript types.
...
updates, AI thinking, AI decisions, negotiation lines, trade results, settlement
events, eliminations, escapes, and dungeon events.
...
\begin{itemize}
  ...
  \item winning grants a coin reward;
...
\textbf{Reserved for Zhiyuan.} This section should describe the final visual
rendering and animation work: PixiJS world rendering, 3D interaction preview,
character/scene modeling choices, animation pipeline, camera/framing,
performance considerations, and screenshots. The rest of this report assumes
this section will be completed after the final visual pass.
}}
...
\begin{itemize}
  ...
  \item \textbf{Developer 2}: boss dungeon feature work, flow polishing, and demo
...
\begin{itemize}
  ...
  \item \textbf{Balance}: dungeon rewards, merchant prices, and survival costs
  may need more playtesting.
  ...
  \item \textbf{Visual completeness}: the rendering section and final screenshots
  should be updated after the last visual pass.
\end{itemize}
...
The final prototype combines a readable survival economy, AI-driven NPC
negotiation, real-time event updates, a polished interaction UI, and a boss
dungeon that gives the presentation a stronger climax. More importantly, the
project shows a practical pattern for future AI games: let the model create
intent and expression, but let typed code own the rules.
